...
建议将 README 中使用的图片统一放在 `images/` 文件夹下，并按如下方式命名：

```text
images/
├── turntable.jpg                 # 高精度三轴转台
├── xw_gi7681.jpg                 # 星网宇达 XW-GI7681 IMU
├── span_100c.jpg                 # NovAtel SPAN-100C IMU
├── acc_x_compensation.png        # X 轴加速度计补偿效果
├── acc_y_compensation.png        # Y 轴加速度计补偿效果
└── alignment_attitude.png        # 静态初始对准姿态角变化
```

这样 GitHub 上的图片引用路径就是：
...
由于部分实验数据文件较大，建议仓库中主要保留代码、说明文档、图片和少量示例数据。
...
- 使用 Git LFS 管理大文件；
- 将完整数据上传到 GitHub Release；
- 将完整数据放入网盘，并在 README 中说明；
- 仓库中只保留小型示例数据。
...
# Keep small example data if needed
!data/example/*.ASC
